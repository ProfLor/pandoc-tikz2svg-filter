...
#
##use pandoc  --katex input.tex -f latex+raw_tex -t gfm --filter=tikz2svg_2.2.py -o output.md to convert TikZ diagrams in LaTeX to SVG images for Markdown/HTML output.
## use the following if you want to have the output file name based on the input file name:
##$in = ".\working\[FILENAME]"
## $out = [IO.Path]::ChangeExtension($in, ".md")
## pandoc --katex $in -f latex+raw_tex  --filter "..\Panflute\tikz2svg_2.2.py" -o $out
    ...
    if isinstance(elem, pf.RawBlock) and elem.format == 'latex':
        ...
        if any(code.startswith(start) for start in ['\\begin{tikzpicture}', '\\begin{circuitikz}', '\\begin{picture}']):
            ...
            if not os.path.exists(outpath_black):
                if not tex2image(elem.text, filetype, outpath_black, CUSTOM_TIKZSET_BLACK):
            ...
            if not os.path.exists(outpath_white):
                if not tex2image(elem.text, filetype, outpath_white, CUSTOM_TIKZSET_WHITE):
            ...
            # Return both blocks; splitting logic above will pull them out of any center wrapper
